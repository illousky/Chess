...
class CreateRoundAPIViewTest(TransactionTestCase):
    ...
    def test_007_create_round_swissByes(self):
        ...
        results = [
            {'white':
             'jrcuesta', 'black': None, 'result': Scores.BYE_U.value},
            {'white': 'ertopo', 'black': None, 'result': Scores.BYE_H.value},
            {'white': 'soria49', 'black': None, 'result': Scores.BYE_H.value},
            {'white':
             'zaragozana', 'black': None, 'result': Scores.BYE_H.value},
            {'white':
             'Clavada', 'black': 'oliva21', 'result':
             Scores.NOAVAILABLE.value},
            {'white':
             'Philippe2020', 'black': 'rmarabini', 'result':
             Scores.NOAVAILABLE.value},
            {'white':
             'jpvalle', 'black': 'eaffelix', 'result':
             Scores.NOAVAILABLE.value},
        ]
        games = Game.objects.all()
        for game, result in zip(games, results):
